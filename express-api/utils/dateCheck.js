...
const dateCheck = async (nip, status) => {
    const absensiData = await AbsensiModel.findOne({ where: { users_nip: nip, status: status }, order: [['createdAt', 'DESC']]});
    const currentDate = (absensiData === null) ? null : new Date(absensiData.createdAt).toLocaleString(["ban", "id"], { day: "numeric" });
